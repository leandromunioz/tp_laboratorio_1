...
int main() {
	...
	do {
		...
		case 3:
			if (flagReading == 0) {
				printf(
						"Se deben cargar los datos antes de realizar cualquier operacion\n");
			} else {
		...
		case 10:
			if (flagReading != 0) {
				if (flagChanges == 0) {
					printf(
							"No se han guardado los archivos, si no los guarda pueden haber problemas");
					printf(
							"1. Guardar en texto\n2. Guardar en binario\n3. Salir\n");
					IngresarEnteroValidado("Ingrese la opcion deseada: ", 1, 3,
							&option);
					switch (option) {
					case 1:
						controller_saveAsText(ARCHIVOTEXTO, employeeList);
